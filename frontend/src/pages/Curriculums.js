...
const Curriculums = () => {
    ...
    return (
        <>
                ...
                <Grid container spacing={2}
                    ...
                    {curriculums?.length > 0 ? (
                        curriculums.map((curriculum) => (
                            <Grid item sx={{maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: '8px'}} xs={12} md={6} lg={6} key={curriculum.id}>
                                ...
                                {curriculum.topics.map((topic) => (
                                    <Card sx={{boxShadow: '5px 5px 15px rgba(0, 0, 0, 0.3)', padding: '8px'}}>
                                        <CardContent sx={{padding: 'unset', margin: 'unset', '&:last-child': {
                                                paddingBottom: '0px', // or any other value
                                            },}}>
                                            <Typography variant="h4" gutterBottom
                                                        sx={{fontSize: 16, fontWeight: 'bold', display: 'flex', justifyContent: 'center', margin: 'unset'}}>
                                                {topic.title}
                                            </Typography>
                                        </CardContent>
                                    </Card>
            ...
            <Button sx={{borderRadius: 8, marginTop: '8px'}}>+</Button>
        </>
    );
};
